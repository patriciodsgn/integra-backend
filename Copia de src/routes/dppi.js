...
router.get('/participacionGeografica', validateParams, async (req, res) => {
    try {
        const { anio, region } = req.query;
        
        const params = {
            anio: anio ? (parseInt(anio) >= 2000 ? parseInt(anio) - 2000 : parseInt(anio)) : null,
            region: region || null
        };

        // Validar que el año sea un número válido si se proporciona
        if (anio && isNaN(params.anio)) {
            return res.status(400).json({
                success: false,
                message: MESSAGES.INVALID_YEAR
            });
        }
        ...
        const result = await request
            .input('Anio', sql.Int, params.anio)
            .input('Region', sql.VarChar(100), params.region)
            .execute('sp_ObtenerParticipacionGeografica');
        ...
        const data = result.recordset.map(item => ({
            ...
            participacion: {
                si: item.Si,
                no: item.No,
                total: item.TotalEncuestas
            },
            porcentajes: {
                si: parseFloat(((item.Si / item.TotalEncuestas) * 100).toFixed(2)),
                no: parseFloat(((item.No / item.TotalEncuestas) * 100).toFixed(2))
            }
        }));
