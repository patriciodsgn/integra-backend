...
router.get('/porcentajeTransitorio', async (req, res) => {
    try {
        const { ano, codigoRegion } = req.query;

        // Validar que el año sea proporcionado y sea válido
        if (!ano || isNaN(parseInt(ano))) {
            return res.status(400).json({
                success: false,
                message: 'El año es un parámetro obligatorio y debe ser un número válido'
            });
        }

        const params = {
            Ano: parseInt(ano),
            CodigoRegion: codigoRegion ? parseInt(codigoRegion) : 0
        };

        // Validar que el código de región sea un número válido si se proporciona
        if (codigoRegion && isNaN(params.CodigoRegion)) {
            return res.status(400).json({
                success: false,
                message: 'El código de región debe ser un número válido'
            });
        }
        ...
        const result = await request
            .input('Ano', sql.Int, params.Ano)
            .input('CodigoRegion', sql.Int, params.CodigoRegion)
            .execute('sp_EducacionObtenerPorcentajeTransitoria');
        ...
        const data = result.recordset.map(item => ({
            region: item.DescripcionRegion,
            comuna: item.Comuna,
            porcentajeTransitorio: item.PorcentajePermanente,
            cantidadTransitoria: item.CantidadPermanente,
